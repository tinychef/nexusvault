...
export function useSync() {
  ...
  const syncAll = useCallback(async (vaultId: string, encryptionKey: Uint8Array) => {
    setStatus("syncing");
    setError(null);
      ...
      for (const remote of remoteDocs) {
        ...
        if (!local || remote.last_modified > local.updatedAt) {
          ...
          if (local) {
            ...
            const localBytes = await readFile(path, { baseDir: BaseDirectory.AppLocalData });
            ...
            await writeFile(path, decryptedBinary, { baseDir: BaseDirectory.AppLocalData });
